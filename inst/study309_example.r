...
f_simulation <- function(i){
  ...
  train.data$TRT = ifelse(train.data$TRT == "bevacizumab with chemotherapy", yes = 0, no = 1)
  ...
  #### fold 1, using -fold1 to train model, using fold1 to test dataset
  # train model
  para.mtry = df[i, "mtry"]
  ...
  if(para.nodedepth == 0)
  {
    ...
    rf.object = tryCatch(expr = {rfsrc(Surv(OS, OSevent)~KRAS+B_ECOG+AGE, 
                                             ...
                                             seed = -(para.seed+j), mtry=para.mtry, 
...
## It can be calculated parallelly.
for (i in 1:dim(df)[1]){
  f_simulation(i)
}

## combine membership folder 
folder_path = "/Users/xli36/Desktop/Rf_analysis/Real_data_analysis/262"
...
### calculate proximity from combined membership
#proximity = rfsrc_compute_proximity(membership = membership, inbag=NULL, oob='ALL')

proximity = rio::import("") ## the calculated proximity can be found from data folder
...
## The final selected decision tree 
plot_profile(result[[1]]$trees[[result[[4]]]])

## The corresponding p leaf is
pleaf = result[[2]]

## We use calibration strategy to exclude null/global scenario.

## if pleaf < p_threshold

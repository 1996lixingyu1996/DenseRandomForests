           ...
           ...)
{
  ...
  if (!restore.mode) {
    ...
    if(treatment_flag)
    {
      newdata <- newdata[, is.element(names(newdata), c(yvar.names[1:2], xvar.names)), drop = FALSE]
    }
    else
    {
  ...
  #  }
    ...
    if (yvar.present) {
      if(treatment_flag)
      {
        yvar.newdata <- as.matrix(newdata[, yvar.names[1:2], drop = FALSE])
      }
      else
      {
  ...
  if (!restore.mode | outcome == "test") {
    ...
    if (perf.type != "none") {
      ...
      if(treatment_flag)
      {
        colnames(yvar.newdata) <- yvar.names[1:2]
      }
      else
      {

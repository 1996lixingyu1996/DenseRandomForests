rfsrc <- function(formula, data, ntree = 500,
                  ...
                  treatment = NULL,  # new code
                  weight_util = NULL,  # new code
                  mini_leaf = 1, # new code
                  den_util = NULL,  # new code
                  seed = NULL,
                  do.trace = FALSE,
                  statistics = FALSE,
                   ...)
{
  ...
  user.option <- list(...)
  ##
  #treatment <- is.hidden.impute.only(user.option)
  #weight_util <- is.hidden.impute.only(user.option)
  #den_util <- is.hidden.impute.only(user.option)
  ...
  if (is.null(treatment)&(!is.null(weight_util))){
    if(length(treatment) == 0){
      stop("treatment is null, please add data!")
    }
  }

  if (!is.null(treatment))
  {
    if (weight_util < 0 || weight_util >1){
      stop("weight_util should range from 0 to 1!")
    }

    if(den_util<=0){
      stop("den_util should larger than 0!")
    }
    ...
    ########
    #cat("weight_util", weight_util);

    ### add weight variale
    weight_util_vec = rep(weight_util, nsample)
    ##

    ### add den_util_weight variable
    den_util_vec = rep(den_util, nsample)
    ##


    ##### new code, add treatment to yvar.names
    yvar.names <- c(yvar.names, c("treatment", "weight_util", "den_util"))
    ####

    ##### new code, extend yvar.wt c(1,1,0)
    yvar.wt = c(yvar.wt, rep(0,3))
    #######

    ##### new code add treatment to yvar
    yvar <- cbind(yvar, treatment, weight_util_vec, den_util_vec)
    #######

    ## extend yvar.types
    yvar.types <- c(yvar.types, rep("R", 3))
    #######

    ## extend yvar.nlevels
    yvar.nlevels <- c(yvar.nlevels, rep(0, 3))

    # tempoary code
    #id = 1:n
    if (mini_leaf < 1){
      stop("mini leaf must larger than 0!")
    }
    mini_leaf_vector <- rep(mini_leaf, nsample)

    yvar.wt <- c(yvar.wt, 0)
    yvar <- cbind(yvar, mini_leaf_vector)
    yvar.types <- c(yvar.types, "R")
    yvar.nlevels <- c(yvar.nlevels, 0)
    yvar.names <- c(yvar.names, c("mini_leaf"))
